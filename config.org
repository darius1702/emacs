...
(keymap-global-unset "C-x m")              ; stop accidentally composing mail
...
(defvar-local darius/mode-line-git-status
  '(:eval
    (when vc-mode
      (if (mode-line-window-selected-p)
          (propertize vc-mode 'face 'darius/mode-line-text)
        ;; explicit shadow face here because standard-themes makes it italic otherwise
...
(use-package emacs
  ...
  :hook
  (modus-themes-post-load . darius/modus-themes-setup)
...
(use-package modus-catppuccin
  :ensure t
  :vc (:url "https://gitlab.com/magus/modus-catppuccin" :rev :newest)
  :custom
  (catppuccin-mocha-palette-overrides
   '((base  "#1c1c1e")
     ...
     (fg-heading-2 mauve)
     (string flamingo)
     (border-mode-line-active overlay0))))
#+end_src
...
(use-package standard-themes
  ...
  :config
  (defun darius/standard-themes-setup ()
    (standard-themes-with-colors
      (set-face-attribute 'font-lock-comment-face nil :foreground fg-dim)
      (set-face-attribute 'font-lock-doc-face nil :foreground fg-dim)
      (set-face-attribute 'font-lock-string-face nil :foreground fg-dim)
      (with-eval-after-load 'org
        (set-face-attribute 'org-level-1 nil :foreground fg-dim :background bg-blue-nuanced :height 1.1)
        (dolist (level (number-sequence 2 8))
          (set-face-attribute (intern (format "org-level-%s" level)) nil :foreground fg-dim :weight 'semibold))
        (set-face-attribute 'org-block nil :background bg-inactive)
        (set-face-attribute 'org-block-begin-line nil :foreground fg-dim :background bg-dim :extend t)
        (set-face-attribute 'org-block-end-line nil :foreground fg-dim :background bg-dim :extend t))))
  :hook
  (standard-themes-post-load . darius/standard-themes-setup))
#+end_src
