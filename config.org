...
** Keybindings
I paid for the whole keyboard so I'm gonna use the whole keyboard
...
(defun darius/scroll-down-command (arg)
...
(defun darius/scroll-up-command (arg)
...
(bind-keys
 ...
 ([remap scroll-up-command] . darius/scroll-up-command)
 ([remap scroll-down-command] . darius/scroll-down-command))
...
#+begin_src emacs-lisp
(defun darius/switch-buffer-or-delete-other-windows (arg)
...
(bind-key "C-;" #'darius/switch-buffer-or-delete-other-windows)
#+end_src
...
#+begin_src emacs-lisp
(defun darius/delete-surrounding-pair (char)
...
(bind-key "C-c D" #'darius/delete-surrounding-pair)
#+end_src
...
#+begin_src emacs-lisp
(defun darius/open-in-other-window (command &optional property)
...
(defun darius/modus-to-foot-palette ()
  ...
  '(:eval
    (when (and (project-current)
      ...
      (let ((project-str (format "%s/" (project-name (project-current)))))
        (if (mode-line-window-selected-p)
            (propertize project-str 'face 'darius/mode-line-text)
          project-str)))))

(dolist (var '(darius/mode-line-buffer-name ; unused
...
(use-package appt
  ...
  (defun darius/send-appt-notification (min-until new-time appt-msg)
    (notifications-notify
     :title "Upcoming Appointment"
     :body (format "In %s min: %s" min-until appt-msg)
     :urgency 'critical
     :timeout 0))
  (setq appt-disp-window-function #'darius/send-appt-notification)
  :custom
...
(use-package package
  ...
  (package-archive-priorities ; prefer stable elpa over devel
   '(("elpa" . 2)
     ("elpa-devel" . 1)))
  :config
  (defun darius/package-upgrade-all ()
...
(use-package textsize
  ...
  (bind-keys ; global map
   ...
   :prefix-map darius/textsize-mode-map
...
(use-package fontaine
  ...
  :config
  (fontaine-mode 1)
  (defvar darius/fontaine-loaded nil "Have we loaded fontaine yet?")
  (defun darius/load-fontaine ()
    (when (display-graphic-p)
      (fontaine-set-preset (or (fontaine-restore-latest-preset) 'regular))))
  (defun darius/load-fontaine-once ()
    "Apply fontaine preset only once since fonts with daemon mode are weird."
    (unless darius/fontaine-loaded
      (darius/load-fontaine)
      (setq darius/fontaine-loaded t)))
  ;; for non-daemon mode
  (darius/load-fontaine)
  (with-eval-after-load 'modus-themes
    (add-hook 'modus-themes-after-load-theme-hook #'darius/load-fontaine))
  :hook
  ...
  (server-after-make-frame . darius/load-fontaine-once))
#+end_src
...
(use-package consult
  ...
  (("C-S-Y"     . consult-yank-from-kill-ring)
   :map search-map
   ("d"     . consult-fd)
   ("D"     . darius/consult-fd-hidden)
   ("l"     . consult-line)
   ("L"     . consult-line-multi)
   ("r"     . consult-ripgrep)
   ("R"     . darius/consult-ripgrep-hidden)
...
Installing this fails because of a package called ~buttercup~ not being present?
...
(use-package elfeed
  ...
  (defun darius/reload-elfeed-feeds ()
    (interactive)
    (when (file-exists-p darius/elfeed-feeds-file)
      (setopt elfeed-feeds
              (with-temp-buffer
                (insert-file-contents darius/elfeed-feeds-file)
                (read (current-buffer))))))
...
(use-package ghostel
  ...
  (defun darius/ghostel-project-other-window ()
    (interactive)
    (darius/open-in-other-window #'ghostel-project :dedicated))
  (defun darius/spawn-ghostel-or-terminal-here (arg)
    "Open a *ghostel* buffer if called without prefix.
If called with prefix arg, spawn a terminal window in the current directory."
    (interactive "P")
    (cond
     ((equal arg '(4))
      (darius/spawn-terminal))
     (t ; otherwise
      (darius/open-in-other-window #'ghostel :dedicated))))
  (defun darius/hide-nbsp ()
  ...
  :hook (ghostel-mode . darius/hide-nbsp)
  :bind (
         :map goto-map
         ("t" . darius/spawn-ghostel-or-terminal-here)
         :map project-prefix-map
         ("t" . darius/ghostel-project-other-window)
...
(use-package eglot
  ...
  :config
  (darius/setup-eglot-keys) ; see below
...
(defun darius/setup-eglot-keys ()
  ...
  (bind-keys :map darius/eglot-map
             ...
             :prefix-map darius/eglot-find-map
  ...
  (bind-keys :map darius/eglot-map
             ...
             :prefix-map darius/eglot-show-map
...
(use-package verilog-mode
  ...
  :bind
  (:map verilog-mode-map ("C-;" . darius/switch-buffer-or-delete-other-windows)))
#+end_src
