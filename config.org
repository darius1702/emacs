...
Disabled because the UX is still trash
...
(use-package modus-alabaster
  :vc (:url "https://github.com/dpassen/modus-alabaster" :rev :newest))
#+end_src
...
If called with prefix arg, spawn a terminal window in the current directory."
  ...
  (defun my/hide-nbsp ()
    "Hide the underline for non-breaking-spaces in the current buffer, this makes the output of for example `tree` look better"
    (setq-local nobreak-char-display nil))
  :hook (ghostel-mode . my/hide-nbsp)
  :bind (
         :map goto-map
