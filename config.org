...
(dolist (key '(("pp" "π")
               ("pP" "Π")))
  (define-key iso-transl-ctl-x-8-map (car key) (cadr key)))
#+end_src
...
(keymap-set goto-map "z" #'darius/zoxide-find-file)
#+end_src
...
(keymap-global-set "C-;" #'darius/switch-buffer-or-delete-other-windows)
#+end_src
...
(keymap-global-set "C-c D" #'darius/delete-surrounding-pair)
#+end_src
...
(use-package hideshow
  ...
  (hs-display-lines-hidden nil) ; TODO set to t when it works with themes
...
(use-package eat
  ...
  (defun darius/spawn-eat-or-terminal-here (arg)
...
If called with two C-u's, spawn a terminal window (foot) in the current directory."
    ...
    (cond
     ...
     (t ; fallback
      (eat-other-window))))
  :bind
  ("M-g t" . darius/spawn-eat-or-terminal-here)
...
(use-package colorful-mode
  ...
  ; colors have a box around them by default which looks weird
...
(use-package eglot
  ...
  (defvar-keymap darius/eglot-map
    :doc "Keymap for eglot commands"
    "TAB" #'eglot-format
    "i"   #'eglot-inlay-hints-mode
    "l"   #'eglot-list-connections
    "r"   #'eglot-rename
    "a"   (cons "actions" darius/eglot-code-actions-map)
    "f"   (cons "find"    darius/eglot-find-map)
    "s"   (cons "show"    darius/eglot-show-map))

  (keymap-set eglot-mode-map "C-c e" darius/eglot-map)
