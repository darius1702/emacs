...
*** Spacious Padding
Some breathing room around frames.
Uses custom-set-faces, so there is some garbage written to the custom file.
...
(use-package evil
  ...
  (dolist (mode '(dired-mode image-mode vterm-mode))
